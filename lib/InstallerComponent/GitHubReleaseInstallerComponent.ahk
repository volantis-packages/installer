class GitHubReleaseInstallerComponent extends DownloadableInstallerComponent {
    ...
    __New(appName, version, repositoryName, releaseFile, zipped, destPath, appState, stateKey, cache, versionSorter, parentStateKey := "", overwrite := false, tmpDir := "", onlyCompiled := false, isTagRelease := false) {
        ...
        super.__New(appName, version, releaseFile, zipped, destPath, appState, stateKey, cache, versionSorter, parentStateKey, overwrite, tmpDir, onlyCompiled)
    }
    ...
    GetParentVersion() {
        ...
        if (response.Has("tag_name")) {
            ...
            if (this._versionSorter.IsOutdated(parentVersion, ghVersion)) {
